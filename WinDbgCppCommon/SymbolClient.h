#pragma once
#include <string>
#include "WinStructres.hpp"
#include <filesystem>

class SymbolClient
{
public:

	static constexpr std::string_view DEFAULT_SERVER =  "https://msdl.microsoft.com/download/symbols";

	SymbolClient(std::filesystem::path file_path);
	SymbolClient();

	std::filesystem::path get_symbol_directory() const;

	void download_pdb(const CV_INFO_PDB70& pdb_info);

private: 
	static std::string get_guid_string(const CV_INFO_PDB70& pdb_info);

	static std::filesystem::path create_temp_path();

	std::filesystem::path m_path;
};

